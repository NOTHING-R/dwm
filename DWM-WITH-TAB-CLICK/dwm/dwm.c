...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			...
			/* advance x past the layout symbol to get start of win title area */
			x += TEXTW(selmon->ltsymbol);

			/* use m->btw (set by drawbar) to find where win titles end and
			 * status text begins — TEXTW(stext) is wrong with status2d ^ codes */
			if (ev->x >= x + m->btw)
				click = ClkStatusText;
			else {
				c = m->clients;
